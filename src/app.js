...
function applyNativeProject(p){
  state.zip=null;state.entries.clear();state.cache.clear();state.projectFiles=p.files||[];state.files=state.projectFiles.filter(x=>x.type==='file').map(x=>x.path);
...
async function openZip(file,rememberedPath=''){
...
function renderAssets(){const q=$('searchAssets').value.toLowerCase();const arr=state.files.filter(k=>k.toLowerCase().includes(q)).slice(0,1000);$('assetCount').textContent=arr.length;$('assetList').innerHTML=arr.length?arr.map(k=>`<div class="asset" data-path="${escapeAttr(k)}" title="${escapeAttr(k)}"><span class="asset-icon">${fileIcon(k)}</span><span>${escapeHtml(k)}</span></div>`).join(''):'<div class="muted">Nenhum arquivo no projeto.</div>';window.__attachFileInteractions?.();}
...
async function saveNinjaProject(){if(state.project?.native&&state.project.filePath&&window.ninjaBridge?.saveProjectFolder){try{const saved=await window.ninjaBridge.saveProjectFolder({filePath:state.project.filePath,name:state.project.name,scene:clone(state.scene)});state.projectFiles=saved.files||[];state.files=state.projectFiles.filter(x=>x.type==='file').map(x=>x.path);rememberProject(state.project.name,state.project.filePath);renderFiles();renderAssets();state.dirty=false;$('dirtyState').textContent='';log('Projeto salvo na pasta: '+state.project.filePath);return;}catch(e){log('Falha ao salvar: '+e.message,'error');}}const blob=new Blob([ninjaProjectText()],{type:'application/json'});download(blob,'project.ninja.json');state.dirty=false;$('dirtyState').textContent='';log('Ninja project model saved.');}
