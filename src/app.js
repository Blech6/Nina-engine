...
$('gridBtn').onclick=()=>{state.grid=!state.grid;$('gridBtn').classList.toggle('active',state.grid);draw();};
...
function descendants(id){const out=[];const q=[id];while(q.length){const p=q.shift();for(const n of state.scene.nodes.filter(n=>n.parent===p)){out.push(n);q.push(n.id);}}return out;}
...
function renderAll(){renderSceneTree();renderInspector();renderFiles();draw();$('sceneName').textContent=state.scene?.name||'Untitled Scene';$('dirtyState').textContent=state.dirty?'● Modified':'';}
function renderSceneTree(){const root=rootForScene(state.scene||{});const rows=[];function walk(parent,depth){const children=state.scene.nodes.filter(n=>n.parent===parent);for(const n of children){rows.push({n,depth});walk(n.id,depth+1);}}rows.push({n:root,depth:0});walk(root.id,1);$('sceneTree').innerHTML=rows.map((r,i)=>`<div class="tree-row ${r.n===state.selected?'selected':''}" data-id="${r.n.id}"><span class="tree-indent">${r.depth?'↳':'▾'}</span><span class="tree-icon">${nodeIcons[r.n.type]||'◇'}</span><span class="tree-label">${escapeHtml(r.n.name)}</span><span class="node-type">${escapeHtml(r.n.type)}</span></div>`).join('');document.querySelectorAll('#sceneTree .tree-row').forEach(el=>el.onclick=()=>{state.selected=state.scene.nodes.find(n=>n.id===el.dataset.id)||null;renderAll();});$('objectCount').textContent=state.scene.nodes.length;}
...
function renderInspector(){const o=state.selected;$('selectedType').textContent=o?o.type:'—';if(!o){$('inspector').innerHTML='<div class="empty-state"><div class="empty-icon">◇</div><strong>Select a node</strong><span>Properties will appear here.</span></div>';return;}const parents=[rootForScene(state.scene),...state.scene.nodes.filter(n=>n.id!==o.id)];$('inspector').innerHTML=`${tileInspector(o)}<div class="property-group"><div class="property-header">NODE</div><div class="property"><label>Name</label><input data-p="name" value="${escapeAttr(o.name)}"></div><div class="property"><label>Type</label><select data-p="type">${NODE_TYPES.map(t=>`<option ${t===o.type?'selected':''}>${t}</option>`).join('')}</select></div><div class="property"><label>Parent</label><select data-p="parent"><option value="">${escapeHtml(rootForScene(state.scene).name)}</option>${parents.filter(p=>p.id&&p.id!==o.id).map(p=>`<option value="${p.id}" ${o.parent===p.id?'selected':''}>${escapeHtml(p.name)}</option>`).join('')}</select></div><div class="property"><label>Source</label><input value="${escapeAttr(o.source||'Internal')}" disabled></div></div><div class="property-group"><div class="property-header">TRANSFORM</div>${['x','y','w','h','rotation','scaleX','scaleY'].map(k=>`<div class="property"><label>${k.toUpperCase()}</label><input type="number" step="0.1" data-p="${k}" value="${o[k]??0}"></div>`).join('')}</div><div class="property-group"><div class="property-header">VISIBILITY</div><div class="property"><label>Visible</label><select data-p="visible"><option value="true" ${o.visible?'selected':''}>True</option><option value="false" ${!o.visible?'selected':''}>False</option></select></div></div><div class="property-group"><div class="property-header">PROPERTIES</div><div class="property-json"><code>${escapeHtml(JSON.stringify(o.properties,null,2))}</code></div></div>`;
  document.querySelectorAll('#inspector [data-p]').forEach(el=>el.onchange=()=>{const before=snapshot(),p=el.dataset.p;if(p==='visible')o[p]=el.value==='true';else if(['x','y','w','h','rotation','scaleX','scaleY'].includes(p))o[p]=Number(el.value);else if(p==='type')o[p]=el.value;else o[p]=el.value;commit(`Change ${p}`,before);renderAll();});
}
function renderFiles(){const q=$('searchFiles').value.toLowerCase();const arr=state.files.filter(k=>k.toLowerCase().includes(q)).slice(0,1000);$('fileList').innerHTML=arr.length?arr.map(k=>`<div class="file-row" title="${escapeAttr(k)}"><span>${fileIcon(k)}</span><span>${escapeHtml(k)}</span></div>`).join(''):'<div class="muted">No matching files.</div>';}
function renderAssets(){const q=$('searchAssets').value.toLowerCase();const arr=state.files.filter(k=>/\.(png|jpe?g|webp|svg|ogg|wav|mp3|ttf|woff2?|tscn|tres|res)$/i.test(k)&&k.toLowerCase().includes(q)).slice(0,500);$('assetCount').textContent=arr.length;$('assetList').innerHTML=arr.length?arr.map(k=>`<div class="asset"><span class="asset-icon">${fileIcon(k)}</span><span title="${escapeAttr(k)}">${escapeHtml(k.split('/').pop())}</span></div>`).join(''):'<div class="muted">No assets found.</div>';}
